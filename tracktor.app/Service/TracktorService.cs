...
namespace tracktor.service
{
    public class TracktorService : ITracktorService
    {
        ...
        public TracktorService(ITracktorContext db, ILog log, IMapper mapper)
        {
            _db = db;
            _log = log;
            _mapper = mapper;
        ...
        public int CreateUser(string userName)
        {
            try
            {
                ...
                var guestEmail = "guest@rohatsu.com";// ConfigurationManager.AppSettings["GuestEmail"];
        ...
        public TSummaryModelDto GetSummaryModel(TContextDto context)
        {
            try
            {
                var calc = new TracktorCalculator(context, _db, _mapper);
            ...
            try
            {
                if (task.TTaskID > 0)
                {
                    ...
                    if (existingTask != null)
                    {
                        ...
                        return _mapper.Map<TTaskDto>(existingTask);
                    }
                }
                else if (task.TProjectID > 0 && !task.IsObsolete && !string.IsNullOrWhiteSpace(task.Name))
                {
                    ...
                    if (existingProject != null)
                    {
                        ...
                        _db.TTasks.Add(newTask);
                        _db.SaveChanges();
                        return _mapper.Map<TTaskDto>(newTask);
                    }
                }
                return null;
            }
            ...
            try
            {
                if (project.TProjectID > 0)
                {
                    ...
                    if (existingProject != null)
                    {
                        ...
                        return _mapper.Map<TProjectDto>(existingProject);
                    }
                }
                else if (!project.IsObsolete && !string.IsNullOrWhiteSpace(project.Name))
                {
                    ...
                    _db.TProjects.Add(newProject);
                    _db.SaveChanges();
                    return _mapper.Map<TProjectDto>(newProject);
                }
                return null;
            }
            catch (Exception /*ex*/)
        ...
        public TEntryDto UpdateEntry(TContextDto context, TEntryDto entry)
        {
            try
            {
                if (entry.TEntryID > 0)
                {
                    ...
                    {
                        var calculator = new TracktorCalculator(context, _db, _mapper);
        ...
        public TReportModelDto GetReportModel(TContextDto context, DateTime? startDate, DateTime? endDate, int projectID, int taskID)
        {
            ...
            {
                var calculator = new TracktorCalculator(context, _db, _mapper);
                return _mapper.Map<TReportModelDto>(calculator.GetReport(startDate, calculator.DateOrLocalNow(endDate), projectID, taskID));
            }
            catch (Exception /*ex*/)
            {
                throw;// new WebFaultException<string>(ex.Message, HttpStatusCode.BadRequest);
